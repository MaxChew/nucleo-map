        ...
        <div class="px-6">
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                    ...
                    <p class="mt-1 text-sm text-gray-600">Update basic information and service settings for the medical center.</p>
                </div>

                <form method="POST" action="{{ route('admin.centers.update', $center) }}" class="px-6 py-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   name="code_name" 
                                   id="code_name" 
                                   value="{{ old('code_name', $center->code_name) }}"
                                   required
                                   maxlength="10"
                        ...
                        <div>
                            ...
                            <select name="state" 
                                ...
                                <option value="W.P. Kuala Lumpur" {{ old('state', $center->state) == 'W.P. Kuala Lumpur' ? 'selected' : '' }}>Kuala Lumpur</option>
                                <option value="Selangor" {{ old('state', $center->state) == 'Selangor' ? 'selected' : '' }}>Selangor</option>
                                <option value="Pulau Pinang" {{ old('state', $center->state) == 'Pulau Pinang' ? 'selected' : '' }}>Penang</option>
                                <option value="Johor" {{ old('state', $center->state) == 'Johor' ? 'selected' : '' }}>Johor</option>
                                <option value="Perak" {{ old('state', $center->state) == 'Perak' ? 'selected' : '' }}>Perak</option>
                                <option value="Sarawak" {{ old('state', $center->state) == 'Sarawak' ? 'selected' : '' }}>Sarawak</option>
                                <option value="Sabah" {{ old('state', $center->state) == 'Sabah' ? 'selected' : '' }}>Sabah</option>
                                <option value="Kelantan" {{ old('state', $center->state) == 'Kelantan' ? 'selected' : '' }}>Kelantan</option>
                                <option value="Pahang" {{ old('state', $center->state) == 'Pahang' ? 'selected' : '' }}>Pahang</option>
                                <option value="Melaka" {{ old('state', $center->state) == 'Melaka' ? 'selected' : '' }}>Melaka</option>
                                <option value="W.P. Putrajaya" {{ old('state', $center->state) == 'W.P. Putrajaya' ? 'selected' : '' }}>Putrajaya</option>
                            </select>
                            @error('state')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   name="code_no" 
                                   id="code_no" 
                                   value="{{ old('code_no', $center->code_no) }}"
                                   required
                                   maxlength="10"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="contact" 
                                      ...
                                      placeholder="Phone, email and other contact information">{{ old('contact', $center->contact) }}</textarea>
                            @error('contact')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="url" 
                                   name="webpage" 
                                   id="webpage" 
                                   value="{{ old('webpage', $center->webpage) }}"
                    ...
                    <div class="mt-6">
                        ...
                        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
                            @php
                                ...
                                $selectedServices = old('services', $center->services ?? []);
                            @endphp
                    ...
                    <div class="mt-6">
                        ...
                        <p class="mt-1 text-sm text-gray-500">Inactive medical centers will not be displayed in the system.</p>
                    </div>
